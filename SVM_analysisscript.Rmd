...
tain_data_balanced_over <- ovun.sample(chd69 ~ age+bmi+lnsbp+dbp+chol+dibpat+smoke+arcus,  
                                  data = train.chd69, 
...
svm2 <- train(factor(chd69) ~ age+bmi+lnsbp+dbp+chol+factor(dibpat)+factor(smoke)+factor(arcus), data = tain_data_balanced_over, method = "svmLinear", trControl = train_control,  preProcess = c("center","scale"), tuneGrid = tunegrid) # might just use the default
#View the model
svm2
```
...
confusionMatrix(data = factor(y_hat1), reference =factor(y_test))
confusionMatrix(data = factor(y_hat3), reference =factor(y_test))
